#!/bin/bash
# ============================================================
# Arch Linux ZEN Gaming Auto-Installer — single file, unattended(ish)
# ============================================================
...
LOGFILE=$([ "$IN_ISO" = 1 ] && echo "/root/arch-install.log" || echo "/var/log/installer.log")
exec > >(tee -a "$LOGFILE") 2>&1

# ========= Ask passwords up front =========
read -rs -p "Password for user 'lied': " PW1; echo
read -rs -p "Confirm password: " PW2; echo
[[ -n "$PW1" && "$PW1" == "$PW2" ]] || die "Passwords empty or mismatch."
...
# ========= Pre-disable fallback BEFORE kernel install =========
log "Pre-disabling fallback initramfs"
...
# pass along passwords for Stage-in-chroot
printf '%s' "$PW1" > /mnt/root/.pw_lied
if [[ "${SETROOT,,}" == "y" ]]; then
  read -rs -p "Root password: " RPW1; echo
  read -rs -p "Confirm root password: " RPW2; echo
  [[ -n "$RPW1" && "$RPW1" == "$RPW2" ]] || die "Root passwords empty or mismatch."
  printf '%s' "$RPW1" > /mnt/root/.pw_root
fi

# ========= Chroot to configure everything =========
log "Entering target system (this will look continuous)…"
arch-chroot /mnt /bin/bash <<'CHROOT'
set -Eeuo pipefail

# -- Enable multilib early (needed for lib32-*, steam, etc.) --
...
# User + passwords from files
id -u lied &>/dev/null || useradd -m -G wheel,audio,video,storage,power,network,optical,scanner,rfkill -s /bin/bash lied
if [[ -f /root/.pw_lied ]]; then printf 'lied:%s\n' "$(cat /root/.pw_lied)" | chpasswd; rm -f /root/.pw_lied; fi
if [[ -f /root/.pw_root ]]; then printf 'root:%s\n' "$(cat /root/.pw_root)" | chpasswd; rm -f /root/.pw_root; else passwd -l root || true; fi
...
# Remove wofi (we use rofi-wayland from AUR)
pacman -Q wofi &>/dev/null && pacman -Rns --noconfirm wofi || true

# NVIDIA KMS + mkinitcpio (fallback already disabled by preset)
...
# Install kernel AFTER config so hook only builds default image
pacman -S --noconfirm linux-zen linux-zen-headers
mkinitcpio -P
rm -f /boot/initramfs-linux-zen-fallback.img || true

# systemd-boot + robust fallback BOOTX64.EFI + make this ESP first
...
# Build yay (AUR) and install AUR packages
...
sudo -u lied yay -S --needed --noconfirm \
  rofi-wayland hyprpicker thunar-vcs-plugin raw-thumbnailer \
...
# Flatpak + Proton-GE via protonup
...
sudo -u lied mkdir -p /home/lied/.config/rofi /home/lied/.config/mako /home/lied/.config/hypr /home/lied/Pictures/Wallpapers /home/lied/.config/ghostty /home/lied/.local/bin /home/lied/.local/share/applications
...
echo -e "[General]\nicon_theme=Papirus-Dark" > /home/lied/.config/qt6ct.conf
echo -e "[General]\nicon_theme=Papirus-Dark" > /home/lied/.config/qt5ct.conf
mkdir -p /home/lied/.config/Kvantum && echo -e "[General]\ntheme=Catppuccin-Mocha" > /home/lied/.config/Kvantum/kvantum.kvconfig
...
# ========= Wrap up & reboot prompt =========
echo
echo "========================================"
echo "Logs: $LOGFILE"
[[ "${WARNINGS:-0}" -gt 0 ]] && echo -e "⚠  Completed with $WARNINGS warning(s)." || echo -e "✅  Completed successfully."
echo "========================================"
